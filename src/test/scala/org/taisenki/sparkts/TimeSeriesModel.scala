...
import com.cloudera.sparkts.models.{ARGARCH, HoltWinters, ARIMA}
...
class TimeSeriesModel {
   ...
   * @param trainTsrdd
   */
  def argarchModelTrain(trainTsrdd:TimeSeriesRDD[String]): RDD[String] ={
    /***参数设置******/
    //往后预测多少个值
    val predictedN=this.predictedN

    /***创建ARGARCH模型***/
    //创建和训练ARGARCH模型.其RDD格式为(ARGARCHModel,Vector)
    val argarchAndVectorRdd=trainTsrdd.map{line=>
      line match {
        case (key,denseVector)=>
          (key,ARGARCH.fitModel(denseVector),denseVector)
      }
    }
    ...
    //预测
    val forecast = argarchAndVectorRdd.map{row=>
      row match{
        case (key,argarchModel,denseVector)=>{
          (key,argarchModel.addTimeDependentEffects(denseVector, predictedVectors))
        }
      }
    }
    val forecastValue=forecast.mapValues(_.toArray.mkString(","))
    forecastValue.collect().map{row=>
      println(s"HoltWinters for key [${row._1}] forecast of next $predictedN observations: ${row._2}")
    }
    return forecastValue.map(_._2)
  }
...
//    dateDataDf.show(30)
    dateDataDf.write.text(s"src/test/resources/data/$outputTableName.csv")
