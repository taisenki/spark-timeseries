package com.cloudera.sparkts.optimize
...
import FirstOrderMinimizerCustom.ConvergenceCheck
...
/**
  ...
  * @author dlwh
  */
abstract class FirstOrderMinimizerCustom[T, DF <: StochasticDiffFunction[T]](val convergenceCheck: ConvergenceCheck[T])(
  ...
  def this(maxIter: Int = -1, tolerance: Double = 1E-6, fvalMemory: Int = 100, relativeTolerance: Boolean = true)(
    implicit space: NormedModule[T, Double]) =
    this(FirstOrderMinimizerCustom.defaultConvergenceCheck[T](maxIter, tolerance, relativeTolerance, fvalMemory))
  ...
  /**
    ...
    */
  type History
  type State = FirstOrderMinimizerCustom.State[T, convergenceCheck.Info, History]
  ...
  protected def initialState(f: DF, init: T): State = {
    ...
    FirstOrderMinimizerCustom.State(x, value, grad, adjValue, adjGrad, 0, adjValue, history, convergenceCheck.initialInfo)
  }
  ...
  def infiniteIterations(f: DF, state: State): Iterator[State] = {
    ...
    Iterator.iterate(state) { state =>
      try {
        ...
        failedOnce = false
        FirstOrderMinimizerCustom.State(
          x,
          value,
          grad,
          adjValue,
...
object FirstOrderMinimizerCustom {
